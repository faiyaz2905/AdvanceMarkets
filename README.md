...
- `app.py` exposes the FastAPI application to Vercel using its supported zero-configuration entry point.
- `vercel.json` owns production routing and security headers.

The obsolete root-level frontend and Express server have been removed so Vercel detects only the FastAPI application. New work must target `frontend/` and `backend/`.
